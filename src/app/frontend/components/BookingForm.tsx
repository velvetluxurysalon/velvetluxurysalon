...
export default function BookingForm({
  ...
  const { user, customerData, isAuthenticated, setShowLoginModal } = useAuth();
  ...
  const getAvailableStaff = () => {
    ...
    const filtered = staff.filter(
      (member) => member.role === selectedService.category,
    );
  ...
  const handleStylistChange = async (
    ...
    if (stylistId && formData.appointmentDate) {
      try {
        const booked = await getBookedSlotsForStylist(
          stylistId,
  ...
  const handleTimeSelect = async (timeValue: string, timeDisplay: string) => {
    ...
    if (bookedSlots.includes(timeValue)) {
      setStylistAvailability(
        `⚠️ This time slot (${timeDisplay}) is already booked. Please select another time.`,
      );
      return;
    }
    ...
    if (formData.stylistId && formData.appointmentDate) {
      try {
        ...
        if (!isAvailable) {
          setStylistAvailability(
            `⚠️ This stylist is already booked at ${timeDisplay}. Please select another time slot.`,
          );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Use customer ID from auth context for consistent phone normalization
      const appointmentData = {
        customerId: customerData?.id || formData.customerPhone, // Use proper customer ID
        customerName: formData.customerName,
        customerEmail: formData.customerEmail,
        customerPhone: customerData?.phone || formData.customerPhone,
            ...
            <div className="p-8 bg-gradient-to-br from-purple-50 to-blue-50 border-2 border-purple-200 rounded-xl text-center">
              <div className="mb-4">
                ...
                <p className="text-gray-600 text-lg mb-4">
                  Sign in to your account to complete your booking with us.
                </p>
              </div>
              <div className="space-y-3">
                <p className="text-sm text-gray-600">
                  It only takes a minute to sign in or create a new account.
                </p>
                <Button
                  onClick={() => {
                          ...
                          {availableSlots.map((slot: any) => {
                            ...
                            return (
                              <button
                                ...
                                onClick={() =>
                                  !isBooked &&
                                  handleTimeSelect(slot.value, slot.time)
                                }
                                disabled={isBooked}
