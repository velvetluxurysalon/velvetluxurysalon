...
export const getServicesGroupedByCategory = async (): Promise<
...
export interface SocialLink {
  id: string;
  platform: string; // 'facebook', 'instagram', 'twitter', 'youtube', etc.
  label: string;
  url: string;
  icon?: string;
  updatedAt?: any;
...
export const getSocialLinks = async (): Promise<SocialLink[]> => {
  try {
    const querySnapshot = await getDocs(
      collection(db, "websiteContent/social/links"),
    );
    return querySnapshot.docs.map((doc) => ({
      id: doc.id,
      ...doc.data(),
...
export const addSocialLink = async (
  ...
  try {
    const docRef = await addDoc(collection(db, "websiteContent/social/links"), {
      ...link,
      updatedAt: serverTimestamp(),
    });
    return docRef.id;
...
export const updateSocialLink = async (
  ...
  try {
    const docRef = doc(db, "websiteContent/social/links", id);
    await updateDoc(docRef, {
      ...link,
      updatedAt: serverTimestamp(),
    });
...
export const deleteSocialLink = async (id: string): Promise<void> => {
  try {
    await deleteDoc(doc(db, "websiteContent/social/links", id));
  } catch (error) {
...
export const getSiteSettings = async (): Promise<SiteSettings | null> => {
  try {
    const docRef = doc(db, "websiteContent", "settings");
    const docSnap = await getDoc(docRef);
...
export const getMembershipById = async (
  ...
  try {
    ...
    const docSnap = await getDoc(docRef);
    return docSnap.exists() ? (docSnap.data() as Membership) : null;
  } catch (error) {
